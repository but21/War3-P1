...
function Challenge:Init()
	...
	ui.panel          = createUI:CreateUIAbsolute("panel", gameui, "中心", 950, 305, 1, 1)
	local w, h        = 70 * 3 + 65, 65
	...
	-- 哈希表[玩家ID] 挑战索引 类别(1挑战次数, 2初始cd, 3当前cd) 值
	local challengeHT = jass.udg_HTChallenge


	for playerID, player in ipairs(jass.udg_Player) do
		for i = 1, 4 do
			-- 挑战次数
			common:SaveInteger(challengeHT[playerID], i, 1, 0)
			-- 初始cd
			common:SaveInteger(challengeHT[playerID], i, 2, excel["其它"][i]["Value3"])
			-- 剩余cd
			common:SaveInteger(challengeHT[playerID], i, 3, excel["其它"][i]["Value3"])
		end
	end
	local function EnterChallengeBtn(btn)
		...
		if index <= 3 then
			local challengeCount = common:LoadInteger(challengeHT[playerID], index, 1)
			local val = excel["其它"][index]["Value4"] + challengeCount * excel["其它"][index]["Value5"]
			tip = tip:gsub("{1}", val)
			tip = tip .. "|n|n挑战成功次数越多，挑战越难，奖励越好！"
		end
	...
	local function ClickChallengeBtn(btn)
		...
		if not ui.cdShadow[index]._show then
			local originCD = common:LoadInteger(challengeHT[playerID], index, 2)
	...
	local function SetChallengeCD()
		...
		for playerID, player in ipairs(jass.udg_Player) do
			for i = 1, 4 do
				local remainCD = common:LoadInteger(challengeHT[playerID], i, 3)
				remainCD = remainCD - 1
				common:SaveInteger(challengeHT[playerID], i, 3, remainCD)
				if remainCD > 0 then
