...
function Card:Init()
	...
	local function addCardsToDrawPool(playerID)
	...
	local function checkSwallow(playerID, bondID)
		local needAmount = excel["羁绊列表"][bondID].NeedAmount
		local ty = excel["羁绊列表"][bondID].SwallowTy
		if ty == 1 then
			if ownedBonds[playerID][bondID] >= needAmount then
				for i = 1, 10, 1 do
